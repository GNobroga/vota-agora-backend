...
let BlockchainTokenService = class BlockchainTokenService {
    ...
    async transferReward(toAddress) {
        try {
            ...
            await this.tokenContract.methods.transfer(toAddress, BlockchainTokenService.REWARD_TOKEN_TO_SEND).send({
                from: accountAddress
            });
            return true;
        } catch  {
    ...
    async registerVote(address, privateKey, publicConsultationId) {
        try {
...
BlockchainTokenService.REWARD_TOKEN_TO_SEND = BigInt(100) * BigInt(10 ** 18);
BlockchainTokenService.DEFAULT_ETHER_TO_NEW_ACCOUNT = 1000;
